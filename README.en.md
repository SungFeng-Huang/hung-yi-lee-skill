...
- **External corpus — plug your own notes into the teacher's graph**: export notes as `raw/external/<collection>/*.md` (frontmatter: required `title`, `source_type`; optional `collection`, `origin_id`, `links`), then `graph build --external` writes a gitignored `wiki/graph/*.local.*` mixed graph (tracked outputs stay lecture-only). `graph query` auto-prefers the mixed graph and tags external nodes with provenance (never presented as lecture content); `links:` cross-references become EXTRACTED edges, cross-document concept co-occurrence becomes INFERRED edges; `model_names:` (curated short names — covers papers whose titles omit the model) become `describes` edges tying a paper doc to its model's concept node, with a conservative dominant-self-mention fallback (skipped for `aggregator: true` curator docs).
- **Extraction-layer triple fix**: `model_names:` + the alignment table form a **dynamic vocabulary** scanned across ALL documents at build time (a lecture mentioning UniAudio now yields the concept — curate once, benefit corpus-wide); a new internal-capital single-token pattern (UniAudio/CosyVoice/WavLM shapes, with a stoplist); and **longest-match-first** extraction ("Classifier-Free Guidance" no longer shatters into "Classifier-Free" + "Free Guidance", and one surface form is no longer double-counted by multiple patterns).
...
- **Source balancing**: `scripts/graph_config.json` — damping for external-corpus edge weights (`auto_balance` derives α=sqrt(transcripts/external) from corpus sizes; ON by default in this fork, set `false` for neutral) — keeps a growing note corpus from drowning the lecture signal.
...
- **Tunable corpus balance**: `external_edge_weight` (α) in `scripts/graph_config.json` — edges carried by external documents are scaled by α and each external doc contributes α of co-mention evidence (low α genuinely de-densifies the paper-side co-occurrence mass); with `auto_balance: true`, α = sqrt(transcript_docs / external_docs) adapts as the paper corpus grows. Default 1.0 = neutral.
